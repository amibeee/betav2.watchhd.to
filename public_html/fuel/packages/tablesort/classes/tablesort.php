	...
	public static function create_table_head()
	{
		...
		foreach(static::$columns as $column)
		{
			...
			if(is_array($column) && (!isset($column[1]) || isset($column[1]) && $column[1] !== false)){
                ...
                // break the url in bits so we can add segments
                $url = parse_url(static::$base_url);
                ...
                // add segments
                $segs[] = static::$current_page ? static::$current_page : '';
                $segs[] = $sort_key.static::$uri_delimiter.$new_direction;
                ...
                $the_url = implode($url).$query;
               		
				$cell_content 	= rtrim(static::$template['link_start'], '> ').' href="'.$the_url.'">';

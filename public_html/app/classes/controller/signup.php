...
class Controller_Signup extends Controller_Base_Public
{
    ...
    public function action_index()
    {
        ...
        if (Input::method() == 'POST')
        {
            ...
            $friend_user_id = trim(Input::post('friend'));

            \Log::error('Received POST data. Username: ' . $username . ', Email: ' . $email . ', Friend User ID: ' . $friend_user_id);
            ...
            if (empty($username))
            {
                ...
                \Messages::warning('Please enter your username.');
            ...
            if (!empty($username) && !preg_match('/^[A-Za-z][A-Za-z0-9]{3,20}$/', $username))
            {
                ...
                \Messages::warning('Username contains invalid characters.');
                \Messages::redirect(Uri::current());
            }
            ...
            if (!empty($username) && Model_User::query()->where('username', '=', $username)->count())
            {
                ...
                \Messages::warning('Username is already taken.');
                \Messages::redirect(Uri::current());
            ...
            if (empty($email))
            {
                ...
                \Messages::warning('Email address is empty.');
            }
            
            if (!empty($email) && !filter_var($email, FILTER_VALIDATE_EMAIL))
            {
                $errors[] = i18n::t('Bitte gebe eine gültige Email Adresse an.');
                \Messages::warning('Invalid email address format.');
            ...
            if (!empty($email) && Model_User::query()->where('email', '=', $email)->count())
            {
                ...
                \Messages::warning('Email address is already taken.');
            }
            ...
            if (Input::post('email', '') != '')
            {
                ...
                \Messages::warning('Spam detected in email field.');
            }
